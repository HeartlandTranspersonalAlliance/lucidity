...
Lucidity does not build or publish an Anaconda installer ISO. The `seed.iso`
attached by the VM harness contains NoCloud first-boot metadata for an existing
QCOW2; it is not installation media. Add ISO boot and unattended Kickstart
installation tests only if a bootc installer ISO or bare-metal installation
path becomes a supported artifact.
