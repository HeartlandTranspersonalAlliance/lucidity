  ...
  perSystem = {
    ...
    infrastructureCheck =
      pkgs.runCommand "lucidity-infrastructure-check" {
        ...
        cp ${source}/nix/infra/tests/aws.tftest.hcl generated-aws/tests/aws.tftest.hcl
        ...
        cp ${source}/nix/infra/tests/state.tftest.hcl generated-state/tests/state.tftest.hcl
    ...
    repositoryCheck =
      pkgs.runCommandLocal "lucidity-repository-policy-check" {
        ...
        if rg -n '^#!/usr/bin/env' \
          ${testSource}/scripts ${testSource}/tests ${testSource}/nix/den/aspects; then
    ...
    runtimeToolsCheck = pkgs.runCommand "lucidity-runtime-tools-check" {} ''
      ...
      ! grep -Fq 'IMAGE_NAME: localhost/lucidity-' ${source}/.github/workflows/validate.yml
      for script in \
      ...
      source_count=$(find ${source}/scripts -maxdepth 1 -type f -name '*.sh' | wc -l)
      ...
      done < <(find ${source}/scripts -maxdepth 1 -type f -name '*.sh' -print)
      ! grep -Fq '$root/scripts/' ${lib.getExe lucidity}
      grep -Fq 'syft.yaml' ${lib.getExe lucidity}
      touch "$out"
    '';
    yamlPolicyCheck =
      pkgs.runCommandLocal "lucidity-yaml-policy-check" {
    ...
    cacheUnitCheck =
      pkgs.runCommandLocal "lucidity-cache-unit-check" {
        ...
        touch "$out"
      '';
    ciWorkflowUnitCheck = pkgs.runCommandLocal "lucidity-ci-workflow-unit-check" {} ''
      touch github.env
      AMI_ROLE=worker \
    ...
    '';
    releaseUnitCheck =
      pkgs.runCommandLocal "lucidity-release-unit-check" {
      ...
      '';
    controllerUnitCheck =
      pkgs.runCommandLocal "lucidity-controller-unit-check" {
        ...
        export LUCIDITY_CONTROLLER_TEST_FIXTURES=${testSource}/nix/den/aspects/controller/tests/fixtures
        mkdir -p "$HOME"
        bash ${testSource}/nix/den/aspects/controller/tests/bootstrap.sh
        touch "$out"
      '';
    workerUnitCheck =
      pkgs.runCommandLocal "lucidity-worker-unit-check" {
        ...
        mkdir -p "$HOME"
        bash ${testSource}/nix/den/aspects/worker/tests/bootstrap.sh
        touch "$out"
      '';
