{
  ...
  nebulaConfig = import ./nebula-config.nix {
    inherit lib role;
  };
  ...
  generatedFiles =
    ...
    // lib.optionalAttrs isController {
      ...
      "usr/libexec/lucidity/bootstrap-controller" =
        builtins.readFile ../../scripts/bootstrap-controller.sh;
      "usr/libexec/lucidity/bootstrap-controller-with-secrets" = ''
        #!/usr/bin/env bash
        set -Eeuo pipefail
        reference_file=/etc/coolify-controller/runtime-secrets.env
        variables=(
        ...
        if [[ ! -e $reference_file ]]; then
          exec /usr/libexec/lucidity/bootstrap-controller
        fi
        ...
        AWS_TOKEN="$token" exec ${asmExec}/bin/asm-exec -- \
          env -u AWS_TOKEN /usr/libexec/lucidity/bootstrap-controller
      '';
      ...
      "usr/libexec/lucidity/prepare-controller-storage" = ''
        #!/usr/bin/env bash
        set -Eeuo pipefail
        install -d -m 0700 /var/lib/coolify /data/coolify
        if ! mountpoint --quiet /data/coolify; then
          mount --bind /var/lib/coolify /data/coolify
        fi
        restorecon -RF /data/coolify
      '';
    ...
    // lib.optionalAttrs (!isController) {
      "usr/libexec/lucidity/bootstrap-worker" = builtins.readFile ../../scripts/bootstrap-worker.sh;
