    ...
    case "${1:-}" in
        controller|worker)
            ...
            if [[ ${LUCIDITY_FULL_GUEST_TEST:-0} == 1 ]]; then
                IMAGE_NAME=$image "$root/scripts/build-disk.sh" "$role" qcow2
                VM_BASE_DISK="$root/image-output/$role/coolify-$role-qcow2.qcow2" \
                    "$root/scripts/vm-init.sh" "$role"
                "$root/scripts/vm-start.sh" "$role"
                trap '"$root/scripts/vm-stop.sh" "$role" || true' EXIT
                "$root/scripts/vm-validate.sh" "$role"
            fi
            ;;
        mesh)
            system=$(nix eval --impure --raw --expr builtins.currentSystem)
...
ci_cache_cleanup() {
    ...
    case "${BUILD_CACHE_ENGINE:-docker}" in
        docker)
            docker logout "$cache_registry" || true
