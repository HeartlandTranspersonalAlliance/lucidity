...
The **Validate controller-worker boot connectivity** workflow classifies the
changed file list first. Role-specific changes build and boot only that disk;
shared or unknown changes build both disks on one GitHub-hosted KVM runner and
boot the guests together. Its cloud-init
