...
- `checks.x86_64-linux.{infrastructure,manifests,policy,secretspec,mesh-vm}`.
...
shell scripts and Actions workflows. CI uses treefmt's fail-on-change mode.
...
commit-pinned input URLs behind. Long image and VM checks emit one-minute
progress messages, record phase durations in the job summary, and retain failure
...
tests/                    supported static, role, and mesh entrypoints
